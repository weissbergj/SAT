/***********************************************************
 * I implemented a pure CDCL SAT solver. It has the following features and justification
 *
...
static int total_conflicts = 0;
static unsigned decision_count = 0;
static unsigned learned_count = 0;
...
int main(int argc, char** argv) {
    ...
    if(!ret) {
        printf("UNSAT\n");
        printf("c conflicts: %u\n", total_conflicts);
        printf("c decisions: %u\n", decision_count);
        printf("c learned: %u\n", learned_count);
        return 0;
    } 
    // final check -> sat
